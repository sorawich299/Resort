...
import { MapPin, Car, Utensils, Star } from "lucide-react"
...
const nearbyData = {
  popular: [
    ...
    { name: "Nongnooch Garden Pattaya", distance: "8.3 km", type: "attraction" },
    { name: "Khao Chee-chan (Buddha Mountain)", distance: "9.3 km", type: "attraction" },
    ...
    { name: "Central Festival Pattaya Beach", distance: "22 km", type: "shopping" },
  ...
  restaurants: [
    ...
    { name: "Anek Kusorn Sala (Viharnra Sien)", distance: "8.3 km", type: "restaurant" },
    { name: "NITAN Coffee's Tale", distance: "11.9 km", type: "coffee" },
  ],
  transportation: [
    { name: "Bali Hai Pier Pattaya", distance: "22 km", type: "pier" },
    { name: "U-Tapao Rayong-Pattaya International Airport", distance: "24 km", type: "airport" },
  ...
  return (
    <div className="p-6 mt-8">
      ...
      <div className="pb-4 mb-6">
        <h3 className="text-2xl font-bold text-gray-800">Nearby</h3>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {nearbyData.popular.map((item, index) => (
              <div
                key={index}
                className="flex flex-col p-2 hover:bg-white rounded-lg transition-all duration-200 cursor-pointer group"
                onClick={() =>
                  setSelectedCategory(selectedCategory === `transport-${index}` ? null : `transport-${index}`)
                }
              >
                <div className="flex gap-2">
                  <span className="text-sm">{getTypeIcon(item.type)}</span>
                  <span className="text-sm text-gray-700 group-hover:text-blue-600 transition-colors">{item.name}</span>
                </div>
                <span className="text-xs text-gray-500 font-medium pl-7">{item.distance}</span>
              </div>
            ))}
          </div>
        </div>
